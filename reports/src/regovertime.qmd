...
#| fig-cap: !expr paste0("Registration over time, ", global_year - 5, "-", global_year)
#| fig-height: 6
#| fig-width: 10
...
p <- all %>%
  pivot_longer(!byvarname, names_to = "year", values_to = "n") %>%
  mutate(byvarname = fct_inorder(byvarname)) %>%
  ggplot(aes(x = year, y = n, group = byvarname)) +
  ...
  scale_y_continuous(breaks = seq(0, 20000, 2000), limits = c(0, 20000), labels = comma_format()) +
...
#| tbl-cap: !expr paste0("Registration over time, ", global_year - 5, "-", global_year)
