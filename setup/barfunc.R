barfunc <- function(var, startime = global_startdtm, stoptime = global_stopdtm, type = "Index",
  ...
  p <- ggplot(data = unitdata, aes(x = countynum, y = percent, fill = !!sym(var))) +
    ...
    guides(fill = guide_legend(nrow = legrows)) +
    scale_y_continuous(breaks = seq(0, 100, 20), limits = c(0, 100.01), expand = c(0, 0)) +
    scale_x_continuous(breaks = c(unique(unitdata$countynum)), labels = percent_ntot$countrypad, expand = c(0, 0), sec.axis = dup_axis(labels = percent_ntot$percent)) +
    labs(y = "Proportion (%)")
  p
}
